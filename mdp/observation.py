...
def cube1_pos_local_obs(env) -> torch.Tensor:
    """Cube1 position in env-local frame (world pos - env_origin). Shape: [N, 3]."""
    cube1 = env.scene["cube1"]
    origins = env.scene.env_origins  # [N, 3]
    return cube1.data.root_pos_w[:, :3] - origins


def cube2_pos_local_obs(env) -> torch.Tensor:
    """Cube2 position in env-local frame (world pos - env_origin). Shape: [N, 3]."""
    cube2 = env.scene["cube2"]
    origins = env.scene.env_origins
    return cube2.data.root_pos_w[:, :3] - origins


def cube1_to_cube2_vec_obs(env) -> torch.Tensor:
    """cube2 - cube1 벡터 (world frame, 환경 origin 차이 상쇄됨). Shape: [N, 3]."""
    cube1 = env.scene["cube1"]
    cube2 = env.scene["cube2"]
    return cube2.data.root_pos_w[:, :3] - cube1.data.root_pos_w[:, :3]


def cube_stack_contact_binary(
    env,
    sensor_cfg: SceneEntityCfg = SceneEntityCfg("contact_sensor_cube1"),
):
    """cube1 ↔ cube2 접촉 여부 (스태킹 판정용). Shape: [N, 1]."""
    sensor = env.scene.sensors[sensor_cfg.name]
    force_matrix = sensor.data.force_matrix_w
    force_mag = torch.norm(force_matrix, dim=-1)
    if force_mag.dim() == 3:
        contact = (force_mag > 1e-6).any(dim=(1, 2))
    elif force_mag.dim() == 2:
        contact = (force_mag > 1e-6).any(dim=1)
    else:
        contact = (force_mag > 1e-6).any(dim=-1)
    return contact.float().unsqueeze(-1)
...
def finger_center_to_cube1_vec_obs(env) -> torch.Tensor:
    """cube1 - finger_center 벡터 (상대 벡터). Shape: [N, 3]."""
    robot = env.scene["robot"]
    cube1 = env.scene["cube1"]
    left_idx = robot.find_bodies("Finger_Left_03")[0][0]
    right_idx = robot.find_bodies("Finger_Right_03")[0][0]
    finger_center = 0.5 * (robot.data.body_pos_w[:, left_idx, :3] + robot.data.body_pos_w[:, right_idx, :3])
    return cube1.data.root_pos_w[:, :3] - finger_center  # [N, 3]
